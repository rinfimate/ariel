...
if (before === after) {
  console.log('[ariel] patch-wasm: nothing to patch');
} else {
  fs.writeFileSync(file, after, 'utf8');
  console.log('[ariel] patch-wasm: replaced import.meta.url with empty string');
}
...
if (fs.existsSync(wasmPkgJson)) {
  ...
  console.log('[ariel] patch-wasm: removed wasm-bindgen/package.json');
}
...
if (fs.existsSync(wasmGitIgnore)) {
  ...
  console.log('[ariel] patch-wasm: removed wasm-bindgen/.gitignore (contained "*", excluded all files from pack)');
}
